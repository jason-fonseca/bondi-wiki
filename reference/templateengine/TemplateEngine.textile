...
You may also sort inside a for loop by using code similar to the following

<pre><code>
   {for myset sort="myfield"}
      //do stuff
   {/for}
</pre></code>

The sort is effectively an ORDER BY clause so you can do sorts as follows
<pre> <code>
{for myset sort="rowid DESC"}
{for myset sort={sortby = "price" ? "price" : ""}}
