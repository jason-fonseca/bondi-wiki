...
- Attachments := A Read-Only array of attachments to send, attachment objects in the array will have properties: Name, ContentType, File. The object File property can be a filename string or stream.
...
- Attachments.Add(Name, ContentType, File) := Add an attachment to the Attachments array to be sent attached to the mail message. File may be either a stream or the path of a file.
