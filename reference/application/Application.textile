...
- Exit := If the bondi process is running in a process pool - calling Exit will signal that the controlling daemon should shut down the entire pool. NOTE: the currently executing script will continue to completion after Application.Exit(); returns - use "Response.End();":https://github.com/ontech/bondi/wiki/Response to stop the script.
