...
var dbconn = new DbConnection("sqlite::" + MerchantID + "_ProductImages::memory"); // memory URI filename
var dbconn = new DbConnection("sqlite::" + MerchantID + "_ProductImages::memory?cache=shared"); //memory URI filename. Allows seperate database connections to share the same in-memory database
...
<pre><code>Db.EvalContext = { 
  PriceIncTax : PriceIncTax
};
//Then in your Query:
"SELECT eval('PriceIncTax(Arguments)',Price, Tax) FROM Products"
</code></pre>
